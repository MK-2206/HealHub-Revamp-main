...
function EditProfile() {
  ...
  const refreshProfileData = async () => {
    try {
      ...
      const response = await fetch("http://localhost:4000/patient/profile/info", {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:4000/patient/edit/info", {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        setError(true);
        throw new Error(errorData.message || "Failed To Edit Profile Info");
      }
